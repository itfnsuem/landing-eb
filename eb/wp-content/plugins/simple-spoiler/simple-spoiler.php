...
Plugin URI: https://wordpress.org/plugins/simple-spoiler/
...
function simple_spoiler_head() {
	global $post;
	wp_register_style( 'simple_spoiler_style', plugins_url( 'css/simple-spoiler.min.css', __FILE__ ), null, '1.0' );
	wp_register_script( 'simple_spoiler_script', plugins_url( 'js/simple-spoiler.min.js', __FILE__ ), array( 'jquery' ), '1.0', true );
...
function simple_spoiler_css() {
	...
	if ( has_shortcode( $post->post_content, 'spoiler' ) ) {
		...
		?>
		<style type="text/css">
			.spoiler-wrap {background: <?php echo esc_attr($spoiler_wrap); ?>;}
			.spoiler-body {background: <?php echo esc_attr($spoiler_body); ?>;}
		</style>
		<?php
	}
}